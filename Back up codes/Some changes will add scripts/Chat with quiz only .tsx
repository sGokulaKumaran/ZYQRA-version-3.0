...
export default function Chat({
  ...
  const createChat = async () => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/create_chat?user_id=1", {
        method: "POST",
      });
  ...
  const deleteChat = async (chatId: number) => {
    await fetch(`http://127.0.0.1:8000/delete_chat/${chatId}`, {
      method: "DELETE",
    });
  ...
  const renameChat = async (chatId: number) => {
    ...
    if (!newName) return;
    await fetch(
      `http://127.0.0.1:8000/rename_chat/${chatId}?title=${encodeURIComponent(newName)}`,
      { method: "PUT" }
    );
    setChats((prev) =>
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "ai", text: "Server error. Please check the backend." },
      ]);
    }
    ...
    <div className="flex h-screen bg-[#0f172a] text-white overflow-hidden">
      ...
      <div className="w-64 bg-[#020617] p-4 flex flex-col h-full flex-shrink-0">
        ...
        <div>
          ...
          {/* Only show New Chat button on chat tab */}
          {activeTab === "chat" && (
            <button
              onClick={createChat}
              className="w-full bg-gray-700 p-2 rounded mb-2 hover:bg-gray-600 text-sm font-medium transition-colors"
            >
              + New Chat
            </button>
          )}
        ...
        {activeTab === "chat" && (
          <div className="flex-1 overflow-hidden flex flex-col mt-4">
            <div className="mb-2 px-1 text-xs text-gray-500 uppercase tracking-wider">
              Recent
            </div>
        ...
        <div className="relative mt-2">
          <button
            ...
            className="w-full bg-gray-800 p-2 rounded hover:bg-gray-700 text-sm transition-colors"
          >
            ⚙️ Settings
          </button>
          ...
          {openSettings && (
            <div
              ref={settingsRef}
              className="absolute bottom-12 left-0 w-full bg-[#1e293b] p-4 rounded-xl shadow-lg border border-gray-700 z-10"
            >
              <h2 className="text-base font-semibold mb-3">Account</h2>
              <p className="text-xs text-gray-400 mb-1">Username</p>
              <p className="bg-gray-800 p-2 rounded mb-3 text-sm">
                {localStorage.getItem("username")}
              </p>
              <button
                onClick={onLogout}
                className="w-full bg-red-600 p-2 rounded hover:bg-red-700 text-sm transition-colors"
              >
                Logout
              </button>
            </div>
      ...
      <div className="flex flex-col flex-1 h-screen overflow-hidden">
              ...
              <div className="w-full max-w-3xl space-y-6">
                ...
                {!currentChat && (
                  <div className="flex flex-col items-center justify-center h-full pt-24 text-center">
                    ...
                    <button
                      onClick={createChat}
                      className="bg-blue-600 hover:bg-blue-700 px-5 py-2 rounded-lg text-sm font-semibold transition-colors"
                    >
                      + New Chat
                    </button>
                  </div>
                        ...
                        components={{
                          ...
                          code({ inline, children }: any) {
                            return inline ? (
                              ...
                              <pre className="bg-black p-3 rounded-lg overflow-x-auto text-sm mb-3">
                                <code>{children}</code>
                              </pre>
                            );
                          },
        ...
        {activeTab === "flash" && (
          <div className="flex-1 flex flex-col items-center justify-center text-center gap-4">
            ...
            <h2 className="text-xl font-semibold text-gray-300">Flashcards</h2>
